...
class SimpleCommandExecutor {
  ...
  isCommandComplete(latestData, allOutput) {
    ...
    const cleanData = data =>
      data.replace(
        /\x1b\[[0-9;]*[mGKHJABCFMPQRSUVWSTfhjlqurwxwzy]|\x1b\[[0-9]*[ABCDHIJKLMPRSTUVWXYZ]|\x1b\[[0-9]*n/g,
        ''
      );
    ...
    const hasPrompt = patterns.some(
      pattern => pattern.test(cleanLatestData) || pattern.test(cleanOutput)
    );

    const hasCommandResult = output.length > latestData.length + 10;
    const hasErrorAndPrompt =
      /不是内部或外部命令|command not found/i.test(cleanOutput) && hasPrompt;
...
export const getCommandHistory = limit => simpleCommandExecutor.getCommandHistory(limit);
