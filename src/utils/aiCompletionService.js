...
请提供5-8个最相关和实用的命令建议。`
  ...
  guessCategory(command) {
    ...
    if (lowerCommand.includes('npm') || lowerCommand.includes('yarn') || lowerCommand.includes('pip')) return 'package'
    ...
    if (['ls', 'cd', 'pwd', 'mkdir', 'rm', 'cp', 'mv', 'find'].some(cmd => lowerCommand.includes(cmd))) return 'file'
    if (['ps', 'top', 'htop', 'kill', 'killall'].some(cmd => lowerCommand.includes(cmd))) return 'process'
    if (['ping', 'curl', 'wget', 'ssh', 'scp'].some(cmd => lowerCommand.includes(cmd))) return 'network'
  ...
  getFallbackSuggestions(input) {
    ...
    if (lowerInput.includes('list') || lowerInput.includes('ls')) {
      fallbackSuggestions.push(
        { command: 'ls -la', description: '显示所有文件包括隐藏文件', confidence: 0.9, type: 'ai', category: 'file' },
        { command: 'ls -lh', description: '以人类可读格式显示文件大小', confidence: 0.8, type: 'ai', category: 'file' },
        { command: 'tree', description: '以树形结构显示目录', confidence: 0.7, type: 'ai', category: 'file' }
      )
    }

    if (lowerInput.includes('install')) {
      fallbackSuggestions.push(
        { command: 'sudo apt install', description: '安装软件包 (Ubuntu/Debian)', confidence: 0.9, type: 'ai', category: 'package' },
        { command: 'sudo yum install', description: '安装软件包 (RHEL/CentOS)', confidence: 0.8, type: 'ai', category: 'package' },
        { command: 'npm install', description: '安装Node.js包', confidence: 0.7, type: 'ai', category: 'package' }
      )
    }
    ...
    if (lowerInput.includes('check') || lowerInput.includes('status')) {
      fallbackSuggestions.push(
        { command: 'systemctl status', description: '检查服务状态', confidence: 0.9, type: 'ai', category: 'service' },
        { command: 'ps aux | grep', description: '查找运行中的进程', confidence: 0.8, type: 'ai', category: 'process' },
        { command: 'netstat -tlnp', description: '检查监听的端口', confidence: 0.7, type: 'ai', category: 'network' }
      )
    }
    ...
    if (fallbackSuggestions.length === 0) {
      fallbackSuggestions.push(
        { command: `${input} --help`, description: '显示命令帮助信息', confidence: 0.6, type: 'ai', category: 'help' },
        { command: `man ${input}`, description: '查看命令手册', confidence: 0.6, type: 'ai', category: 'help' }
      )
    }
