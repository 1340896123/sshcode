...
electron_1.contextBridge.exposeInMainWorld('electronAPI', {
  ...
  sshExecute: (connectionId, command) =>
    electron_1.ipcRenderer.invoke('ssh-execute', connectionId, command),
  sshDisconnect: connectionId => electron_1.ipcRenderer.invoke('ssh-disconnect', connectionId),
  // SSH Shell sessions
  sshCreateShell: (connectionId, options) =>
    electron_1.ipcRenderer.invoke('ssh-create-shell', connectionId, options),
  sshShellWrite: (connectionId, data) =>
    electron_1.ipcRenderer.invoke('ssh-shell-write', connectionId, data),
  sshShellResize: (connectionId, rows, cols) =>
    electron_1.ipcRenderer.invoke('ssh-shell-resize', connectionId, rows, cols),
  sshShellClose: connectionId => electron_1.ipcRenderer.invoke('ssh-shell-close', connectionId),
  // File operations
  getFileList: (connectionId, remotePath) =>
    electron_1.ipcRenderer.invoke('get-file-list', connectionId, remotePath),
  uploadFile: (connectionId, localPath, remotePath) =>
    electron_1.ipcRenderer.invoke('uploadFile', connectionId, localPath, remotePath),
  uploadDroppedFile: (connectionId, file, remotePath) =>
    electron_1.ipcRenderer.invoke('uploadDroppedFile', connectionId, file, remotePath),
  selectAndUploadFile: (connectionId, remotePath) =>
    electron_1.ipcRenderer.invoke('selectAndUploadFile', connectionId, remotePath),
  downloadFile: (connectionId, remotePath) =>
    electron_1.ipcRenderer.invoke('downloadFile', connectionId, remotePath),
  downloadAndOpenFile: (connectionId, remotePath) =>
    electron_1.ipcRenderer.invoke('downloadAndOpenFile', connectionId, remotePath),
  // File monitoring
  startFileWatcher: (remotePath, localPath) =>
    electron_1.ipcRenderer.invoke('startFileWatcher', remotePath, localPath),
